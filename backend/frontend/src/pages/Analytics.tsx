    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-slate-800 rounded-lg border border-slate-700 p-5">
          ...
          <p className="text-xs text-slate-400 mb-3">A = top 70% volume ? B = next 20% ? C = bottom 10%</p>
          {abcData.length === 0 ? (
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 p-5 mb-8">
        ...
        <p className="text-xs text-slate-400 mb-4">Total outbound quantity per product (fast movers first)</p>
        {velocity.length === 0 ? (
          <p className="text-slate-400 text-sm">No outbound movement data yet.</p>
        ) : (
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={velocity} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis type="number" stroke="#94a3b8" />
              <YAxis dataKey="product_name" type="category" stroke="#94a3b8" width={130} tick={{ fontSize: 11 }} />
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 p-5 mb-8">
        ...
        {forecastChartData.length === 0 ? (
          <p className="text-slate-400 text-sm">No forecast data yet. Add stock movements first.</p>
        ) : (
      ...
      {summary?.model_accuracy_distribution && (
        <div className="bg-slate-800 rounded-lg border border-slate-700 p-5">
          ...
          <p className="text-xs text-slate-400 mb-4">R-squared based accuracy labels across all forecasted products</p>
