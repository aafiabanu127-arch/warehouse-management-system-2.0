  ...
  return (
    <div className={`min-h-screen flex items-center justify-center px-4 py-8 transition-colors duration-300 ${d ? 'bg-[#0a0f1a]' : 'bg-slate-100'}`}>
      ...
      <div className={`w-full max-w-4xl flex flex-col lg:flex-row rounded-2xl overflow-hidden shadow-2xl border transition-colors duration-300 ${d ? 'border-white/10' : 'border-slate-200'}`}>
        ...
        {/* Left Panel */}
        <div className="hidden lg:flex lg:w-1/2 bg-[#0C2340] flex-col justify-between p-10 relative overflow-hidden">
          <div
          ...
          <div className="relative z-10">
            ...
            <p className="text-[#85B7EB] text-sm leading-relaxed">
              Centralized inventory control,<br/>zone tracking & AI-powered demand forecasting.
            </p>
          </div>
          <div className="relative z-10 grid grid-cols-2 gap-3">
            {[
              { num: '7', label: 'User roles' },
              { num: 'AI', label: 'Space optimizer' },
              { num: '360°', label: 'Stock visibility' },
              { num: 'REST', label: 'API powered' },
            ].map(({ num, label }) => (
              <div key={label} className="bg-white/[0.06] border border-white/10 rounded-xl px-4 py-3">
                <div className="text-[#E6F1FB] text-lg font-semibold">{num}</div>
                <div className="text-[#85B7EB] text-[11px] uppercase tracking-wide mt-0.5">{label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Right Panel */}
        <div className={`w-full lg:w-1/2 px-6 py-10 sm:px-10 flex flex-col justify-center transition-colors duration-300 ${d ? 'bg-[#111827]' : 'bg-white'}`}>
          ...
          <div className="mb-7">
            ...
            <h2 className={`text-2xl font-semibold ${d ? 'text-white' : 'text-slate-800'}`}>Sign in to your account</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="login-username" className={`block text-xs font-medium uppercase tracking-wide mb-2 ${d ? 'text-slate-400' : 'text-slate-500'}`}>
                Username
              </label>
              <div className="relative">
                <span className={`absolute left-3 top-1/2 -translate-y-1/2 ${d ? 'text-slate-500' : 'text-slate-400'}`}>
                ...
                <input
                  ...
                  className={`w-full pl-9 pr-4 py-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 transition ${d
            ...
            <div>
              <label htmlFor="login-password" className={`block text-xs font-medium uppercase tracking-wide mb-2 ${d ? 'text-slate-400' : 'text-slate-500'}`}>
                Password
              </label>
              <div className="relative">
                <span className={`absolute left-3 top-1/2 -translate-y-1/2 ${d ? 'text-slate-500' : 'text-slate-400'}`}>
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className={`w-full pl-9 pr-10 py-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 transition ${d
                    ? 'border border-white/10 text-white bg-white/5 placeholder-slate-600 focus:ring-blue-500/40 focus:border-blue-500/50'
                    : 'border border-slate-200 text-slate-800 bg-slate-50 placeholder-slate-400 focus:ring-blue-500/30 focus:border-blue-400'}`}
                />
                <button
                  ...
                  className={`absolute right-3 top-1/2 -translate-y-1/2 transition ${d ? 'text-slate-500 hover:text-slate-300' : 'text-slate-400 hover:text-slate-600'}`}
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" className={`w-4 h-4 rounded focus:ring-blue-500 ${d ? 'border-slate-600 bg-white/5 text-blue-500' : 'border-slate-300 text-blue-600'}`}/>
                <span className={`text-sm ${d ? 'text-slate-400' : 'text-slate-500'}`}>Keep me signed in</span>
              </label>
              <Link to="/forgot-password" className={`text-sm hover:underline transition ${d ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'}`}>
                Forgot password?
              </Link>
            </div>
          ...
          <div className={`border rounded-xl p-4 ${d ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-100'}`}>
            <p className={`text-xs font-medium mb-2 ${d ? 'text-slate-400' : 'text-slate-600'}`}>Your dashboard adapts to your role</p>
            <div className="flex flex-wrap gap-1.5">
              {['Admin','Manager','Supervisor','Staff','Picker','Auditor','Viewer'].map(role => (
                <span key={role} className={`text-[11px] px-2.5 py-1 rounded-full border ${d ? 'bg-blue-900/50 text-blue-300 border-blue-800/60' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>
                  {role}
                </span>
              ))}
            </div>
          ...
          <p className={`mt-6 text-center text-sm ${d ? 'text-slate-500' : 'text-slate-400'}`}>
            ...
            <Link to="/register" className={`font-medium hover:underline transition ${d ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'}`}>
              Create account
            </Link>
          </p>
        </div>
